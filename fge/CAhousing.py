...
def build_model(seed=8, eta=0.3, max_depth=8, subsample=1.0, num_rounds=200, save=True):
    """
    ...
    if save:
        if not cache_path.exists():
            cache_path.mkdir()
        print(cache_path)
        with (cache_path / 'CAmodel.pickle').open('wb') as file:
            pickle.dump(res, file)
    else:
        return res

if __name__ == "__main__":
    build_model(seed=8, eta=0.3, max_depth=8, subsample=1.0, num_rounds=200, save=True)